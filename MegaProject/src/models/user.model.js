  ...
  const accessToken = jwt.sign(
        {
          user: {
            //PayLoad for JWT
            _id: this._id,    //jwt_variable_name: data
            email: this.email,
        ...
        { expiresIn: jwtExpiry } // option for JWT expiration, giving 'expiresIn' is necessary, as we want to make sure that after this time the token must be deleted automatically
      );
      return accessToken
}
//'.sign({payLoad}, signature, {options})' --> jwt method to create a jwt token, hum payLoad and signature provide karte hai aur jwt uske basis par ekk jwt token generate karke humme deta hai, hum along with it kuch additional options bhi delte hai (like: expiresIn), jinke through hum token kii kuch properties ko control kar sakte hai)

userSchema.methods.generateRefreshToken = function(){
  const tokenSecretKey = process.env.REFRESH_TOKEN_SECRET
  const tokenExpiry = process.env.REFRESH_TOKEN_EXPIRY
  ...
  const refreshToken = jwt.sign(
        ...
        tokenSecretKey, //Signature for JWT
        { expiresIn: tokenExpiry } // option for JWT expiration, giving 'expiresIn' is necessary, as we want to make sure that after this time the token must be deleted automatically
      );
      return refreshToken
}
...
// Access Token -> A short-lived (for 5–15 min) credential used to access protected resources, its purpose is to prove whether the user is authenticated or not, and it also carries authorization info (like: roles, permissions, etc)
// Refresh Token -> A long-lived (for days/weeks) credential used only to get new access tokens, its purpose is to keep user logged in without re-entering credentials
